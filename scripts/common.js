...
function checkValuesNum(formElem,iC=false,iSp=false,iFilter=false) 
{

	var score = 0;
	var form = getForm(formElem,iFilter);
	for (var i=0; i<getQCount(); i++) 
	{
//		var userAns = parseFloat(form.elements[i].value);
//		if (form.elements[i].value != null && isNaN(userAns))
		var userAns = parseFloat(form[i].value);
		if (form[i].value != null && isNaN(userAns))
		{
			var att = attempts;
			checkValuesMulWays(form,iC,iSp,iFilter);
			attempts=att;
			continue;
		}
...
function getForm(fElem,iFl)
{
var form=[];
	// Extract user Answers into an array
	if(iFl) //If filtering requested
	{
...
function checkValuesMulWays(formElem,iC=false,iSp=false,iFilter=false) 
{
	...
	{
		if(isNaN(parseFloat(userAns[i]*1)))
		{
	...
	for (var i=0; i<answers.length; i++) 
	{
		...
		var ans = answers[i];
		var found = 0;
//		if( !isNaN(parseFloat(answers[i])))
		if( !isNaN(answers[i]*1) && ( !isNaN(userAns[i]*1)))
		{
//			if(!isNaN(parseFloat(userAns[i])))
			{
...
function toPower (zEvent,ed) {
   ...
   // alert(exChar);
        	...
        	var sp = ed.selectionStart;
        	//insert power value at the currect position
        	ed.value=ed.value.substring(0,sp)+suPwr+ed.value.substring(sp);
